...
### Using `compile_facts_to_powershell/3` (Class-Based Export)

For standalone fact export without the function-based approach, use `compile_facts_to_powershell/3`:
...
class PARENT {
    <#
    .SYNOPSIS
    Exported Prolog facts for parent/2
    #>
...
**Benefits:**
- Self-contained PowerShell class with .NET types
- `GetAll()`, `Stream()`, and `Contains()` static methods
- Works with PowerShell 5.1+ class syntax
