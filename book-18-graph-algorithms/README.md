...
| B | [UnifyWeaver-internal theory beyond the main text](13_appendix_b_internal_theory.md) | The (graph, metric)-pair framing; statistical homogeneity (H1/H2/H3); weights as path-count normalisers; topical scoping for homogeneity; symmetric-DAG observation; routing-correction redundancy; convergence robustness (feature + trap); geometric-vs-metric decoupling; inhomogeneity gap; data-prep consequences; extensions to undirected/infinite; spectral expansion connection; `r = b'/(b_eff·D)` as contraction-rate conjecture and tracked theory work. Pointers into `docs/design/TREE_LIKENESS_INDEX*.md` and `docs/design/RECURRENCE_EVALUATION_STRATEGY_*.md`. |
