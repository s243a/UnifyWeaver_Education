...
## Bash Background: Subshells and Pipes

Before diving into the problem, let's understand two key bash concepts.

### What is a Subshell?

A **subshell** is a child process running a copy of bash. It inherits copies of all variables from the parent, but changes in the subshell don't affect the parent.

```bash
x=1
( x=2; echo "In subshell: $x" )   # Outputs: 2
echo "In parent: $x"               # Outputs: 1 (unchanged!)
```

The parentheses `()` explicitly create a subshell.

### Why Do Pipes Create Subshells?

When you use a pipe (`|`), bash needs both sides to run simultaneously so data can flow. To do this, it runs the right side in a subshell:

```
[Parent Shell]  ──pipe──>  [Subshell]
    x=1                      x=2
                           (writes to x here)
    x still 1!             (changes lost when done)
```

This enables parallelism but means variables modified after the pipe don't persist.

### Associative Arrays Quick Reference

UnifyWeaver uses bash associative arrays (hash maps) extensively:
...
# Check if key exists
[[ -n "${colors[sky]}" ]] && echo "exists"

# All keys
echo ${!colors[@]}        # sky grass

# Global array (persists across functions)
