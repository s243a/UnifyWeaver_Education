...
## Exporting Facts with `compile_facts_to_sql/3`

The `compile_facts_to_sql/3` API exports Prolog facts directly to SQL CREATE TABLE and INSERT statements:

```prolog
?- assertz(color(red)).
?- assertz(color(green)).
?- assertz(color(blue)).
?- sql_target:compile_facts_to_sql(color, 1, Code).
```
...
CREATE TABLE IF NOT EXISTS color (arg1 TEXT);
...
This is useful for:
- Migrating Prolog knowledge bases to SQL databases
